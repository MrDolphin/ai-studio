...
function Orbs() {
  ...
  useFrame(() => {
    ...
    for (const orbId in gs.orbs) {
      ...
      const scale = orb.type === 'super' ? 1.5 : orb.type === 'bonus' ? 2.0 : 1.0;
...
export function GameScene() {
  ...
  useFrame((state, delta) => {
    ...
    const { sendCutTail } = useGameStore.getState();
    ...
    if (serverPlayer && serverPlayer.state === 'alive') {
      ...
      for (const otherId in gs.players) {
        ...
        if (collided || cutTargetId) break;
      }

      if (cutTargetId && cutSegmentIndex !== -1) {
        sendCutTail(cutTargetId, cutSegmentIndex);
        // Predict locally
        const target = gs.players[cutTargetId];
        if (target) {
          target.segments = target.segments.slice(0, cutSegmentIndex);
